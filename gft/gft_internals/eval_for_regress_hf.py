...
def my_eval(args, eqn, accelerator, datasets):

    # print('calling my_eval in eval_for_regress_hf.py')
    assert eqn['eqn_type'] == parse_eqn.eqn_types['regress'], 'expected eqn_type: regress, but eqn_type is: ' + str(eqn['eqn_type'])
    ...
    #     tokenizer = AutoTokenizer.from_pretrained(base_model_key)
    # else: 
    ...
    eval_metric = eval_loop_with_regression()
    if fig is None:
