...
def fit(args, eqn, accelerator, raw_datasets, is_regression=False):
    ...
    print('calling fit in fit_for_classify_pd.py (PaddleHub version)')

    if is_regression:
        assert eqn['eqn_type'] == parse_eqn.eqn_types['regress'], 'fit_for_classify is for eqn_type: regress, but eqn_type is: ' + str(parse_eqn.eqn_types_list[eqn['eqn_type']])
    else:
        assert eqn['eqn_type'] == parse_eqn.eqn_types['classify'], 'fit_for_classify is for eqn_type: classify, but eqn_type is: ' + str(parse_eqn.eqn_types_list[eqn['eqn_type']])
    ...
    print('batch_size: ' + str(batch_size), file=sys.stderr)
    ...
    ): [data for data in fn(samples)]   # fn(samples) -- changed by kwc
