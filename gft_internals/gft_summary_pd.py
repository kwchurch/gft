...
def summarize_dataset_pd(args, config=None):
    ...
    from gft_internals.paddle_dataset_zoo import paddle_dataset_infos
...
def infer_models_from_substring_pd(args, s):
    from gft_internals.paddle_model_zoo import paddle_model_zoo
    matches = [m for m in paddle_model_zoo if s in m.lower()]
    print('\n'.join(matches))

def infer_datasets_from_substring_pd(args, s):
    from gft_internals.paddle_dataset_zoo import paddle_datasets
