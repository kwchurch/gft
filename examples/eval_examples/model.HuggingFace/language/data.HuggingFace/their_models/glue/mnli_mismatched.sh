...
for model in ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli roberta-large-mnli 
...
do
    # echo "MODEL $model"
    gft_eval --data H:glue,mnli --eqn 'classify: label ~ premise + hypothesis' --splits train,validation_mismatched,test_matched  --split val  --model H:$model --metric H:glue,mnli --debug
done
