...
do
    echo "MODEL $model"
    gft_predict --data P:glue,mnli --eqn 'classify: labels ~ sentence1 + sentence2' --splits train,dev_matched,test_matched  --split val --model $model  # >$out/predict.out 2>$out/predict.err
done
